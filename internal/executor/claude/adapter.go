...
// Adapter Claude is compatible with the stream-json CLI adapter
type Adapter struct {
	mu          sync.Mutex
	cmd         *exec.Cmd
...
// NewAdapter creates Claude adapter
func NewAdapter() executor.Adapter {
	return NewStreamJSONAdapter(defaultDisplayName)
}

// NewStreamJSONAdapter creates a CLI adapter compatible with Claude's stream-json protocol.
func NewStreamJSONAdapter(displayName string) executor.Adapter {
	displayName = strings.TrimSpace(displayName)
	if displayName == "" {
		displayName = defaultDisplayName
	}
	return &Adapter{displayName: displayName}
}

// NewConversation New conversation: claude -p --output-format stream-json
func (a *Adapter) NewConversation(ctx context.Context, opts executor.RunOptions) (<-chan executor.ExecutorEvent, error) {
	args := []string{"-p", fullPermissionArg, "--verbose", "--output-format", "stream-json"}
...
// ResumeConversation Continue the conversation: claude -p --resume {session_id} --output-format stream-json
...
func (a *Adapter) runCommand(ctx context.Context, opts executor.RunOptions, args []string) (<-chan executor.ExecutorEvent, error) {
	...
	// Prompt must be passed via stdin. Under Windows, claude is usually packaged by claude.cmd,
	// Putting multi-line Prompt into command line parameters will be reinterpreted by cmd.exe according to newlines or special characters.
	// As a result, Claude only receives the first line, and subsequent stream-json parameters may also be lost.
	...
	//Continue to read and retain stderr; when exiting with non-zero value, the real reason will be passed to the task execution window.
	...
	go func() {
		...
		for scanner.Scan() {
			...
			case "assistant":
				for _, block := range event.Message.Content {
					...
					case "tool_use":
						content := block.Name
						if input := claudeRawText(block.Input); input != "" {
				...
				if content := claudeRawText(event.Content); content != "" {
					eventCh <- executor.ExecutorEvent{
						Type:      executor.EventMessage,
						Content:   content,
						SessionID: sessionID,
				...
				if len(event.PermissionDenials) > 0 {
					...
					for _, denial := range event.PermissionDenials {
						requests = append(requests, executor.PermissionRequest{
							...
							ToolInput: claudeRawText(denial.ToolInput),
						})
					}
					eventCh <- executor.ExecutorEvent{
						Type:               executor.EventPermission,
						Content:            summarizePermissionRequests(a.displayLabel(), requests),
						SessionID:          sessionID,
		...
		// Wait for the process to exit and combine result, stderr and exit code into a single final error.
		waitErr := cmd.Wait()
		stdinErr := <-stdinDone
		a.clearProcess(cmd)
		...
		if !sawResult {
			failureDetails = append(failureDetails, a.displayLabel()+" 未输出有效的 result 结束事件")
		...
		if len(failureDetails) > 0 || stdoutScanErr != nil || waitErr != nil {
			eventCh <- executor.ExecutorEvent{
				...
				Error:     formatExecutionError(a.displayLabel(), strings.Join(failureDetails, "\n"), stderrDiagnostic.String(), stdoutScanErr, waitErr),
				SessionID: sessionID,
				Timestamp: nowMillis(),
			}
			return
...
func summarizePermissionRequests(displayName string, requests []executor.PermissionRequest) string {
...
func formatExecutionError(displayName, resultError, stderrText string, stdoutScanErr, waitErr error) string {
...
// claudeEvent Claude stream-json event
...
func (a *Adapter) Stop() error {
	...
	// Root PID, the child process started by Claude will become an orphan and continue to run.
